...
for line in sys.stdin:
	if not line.startswith('bp_overlap'):
		...
		l1 = int(contents[2])
		l2 = int(contents[5])
		if 'repeat_region' in contents[1] or 'repeat_region' in contents[4]:
			if not ('repeat_region' in contents[1] and 'repeat_region' in contents[4]):
				if 'repeat_region' in contents[1]:
					print(contents[4])
					continue
				if 'repeat_region' in contents[4]:
					print(contents[1])
					continue
