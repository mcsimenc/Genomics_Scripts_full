...
if '-h' in sys.argv:
	print('''
			usage:
				gffModifyAttr.py -map <file> < <file.gff> > output.gff


			description: For modifying Santalales CP genome MAKER gff.
				     Replaces the part of any value in the attributes
				     field that is: exonerate_protein2genome-gene
				     with the second field of the -map file for which
				     the first column matches the gene name which is
				     defined as matching this regular expression:

				     (.+?exonerate_protein2genome-gene-[\d]\.[\d]+)

				     Genes found to not have anotations
